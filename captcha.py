...
class Captcha(object):
    ...
    def solver(self) -> str:
        ...
        image = cv2.imread(self.local_file)
    ...
    def filter_allowed_characters(self, text:str) -> str:
        self.logger.info('Filtering only valid characters.')
        filter = re.findall('[a-z0-9]', text)
        ret = ''.join(filter)
        self.logger.info(f'cleaned: {ret}')
        return ret

if __name__ == "__main__": # pragma: no cover
    captcha = Captcha() # pragma: no cover
    ...
    captcha.create_img_file(imgstring=imgstring) # pragma: no cover
    text = captcha.solver() # pragma: no cover
    text = captcha.filter_allowed_characters(text=text) # pragma: no cover
